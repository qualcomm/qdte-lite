...
# Major Version 2 introduces a change to the meta data layout, introducing CHIPINFO and PLATFORMINFO fields
# In order to maintain backward compatibility, certain parts of the code need to know if Major Version is before this change
INFO_VERSION = 2

MAJOR_VERSION_CFG,MINOR_VERSION_CFG = 0x02,0x00
major_version = MAJOR_VERSION_CFG
...
cfg_item_json = cfg_item_json_v1
version_dict = {}

def align_to_alignsize(original_size,align_bytes=0x1):
...
def print_meta_data_table(meta_header, meta_cfg_entries):
  print("\nprint_meta_data_table..major_version[" + str(major_version) + "]\n")
  print("\n-----------------------------------------------------XBL Config Meta Header-----------------------------------------------------")
  print(('{:>13} {:>17} {:>17} {:>24} {:>18}'.format(*list(meta_header.keys()))))
  print_string=[]
  ...
  if major_version < INFO_VERSION:
    print(('{:>10} {:>26} {:>12} {:>18} {:>26} {:>18}'.format(*list(meta_cfg_entries[0].keys()))))
  else:
    print(('{:>10} {:>26} {:>12} {:>18} {:>18} {:>18} {:>26} {:>18}'.format(*list(meta_cfg_entries[0].keys()))))
...
def write_num_little_endian(wt_size, bin_fp, data):
  ...
  for i in range (0,wt_size):
    ...
    if sys.version_info >= (3, 0, 0):
      temp = temp.to_bytes(1,byteorder='little',signed=False)
    else:
      temp = chr(temp)
    bin_fp.write(temp)
...
def read_meta_header(bin_fp):
  ...
  for header_field in list(meta_header.keys()):
    ...
    if str(header_field) == MAJOR_VERSION:
      major_version = int(meta_header[MAJOR_VERSION][VALUE], 0)
      version_dict[MAJOR_VERSION] = major_version
      if major_version < INFO_VERSION:
        cfg_item_json = cfg_item_json_v1
      else:
        cfg_item_json = cfg_item_json_v2
    if str(header_field) == MINOR_VERSION:
      minor_version = int(meta_header[MINOR_VERSION][VALUE], 0)
      version_dict[MINOR_VERSION] = minor_version
...
def calc_and_fill_cfg_entry_offset_and_meta_data_size(meta_cfg_entries, meta_header, align_bytes=0x1):
  ...
  for entry_index in range(1, len(meta_cfg_entries)):
    meta_cfg_entries[entry_index][OFFSET_FROM_META_START][VALUE] = hex(int(meta_cfg_entries[entry_index-1][OFFSET_FROM_META_START][VALUE],16) + align_to_alignsize(meta_cfg_entries[entry_index-1][ITEM_SIZE][VALUE],align_bytes))
...
def create_new_cfg_item(input_cfg_item, base_directory, align, store_original_size = False):
  config_file = None
  cfg_name = None
  cfg_entry = json.loads(cfg_item_json, object_pairs_hook=OrderedDict)
  ...
  # populate CHIPINFO using input from .json if it exists
    if CHIPINFO in input_cfg_item:
      ...
      cfg_entry[CHIPINFO][VALUE] = "0xFFFFFFFFFFFFFFFF".rstrip("L")
  # populate PLATFORMINFO using input from .json if it exists
    if PLATFORMINFO in input_cfg_item:
      cfg_entry[PLATFORMINFO][VALUE] = input_cfg_item[PLATFORMINFO].rstrip("L")
    else:
      cfg_entry[PLATFORMINFO][VALUE] = "0xFFFFFFFFFFFFFFFF".rstrip("L")
  # populate file_name and config-item size using input from .json
  if FILE_NAME in input_cfg_item:
    #if file_name in input_cfg_item is absolute, use it directly.
    if os.path.isfile(str(input_cfg_item[FILE_NAME])):
      config_file = str(input_cfg_item[FILE_NAME])
    #if file_name in input_cfg_item is relative, join it with base directory
    else:
  ...
  if CONFIG_NAME in input_cfg_item:
    ...
    unaligned_bytes = (int(cfg_entry[ITEM_SIZE][VALUE],16)) % int(align,16)

    if(unaligned_bytes):

      alignment_bytes = int(align,16) - unaligned_bytes

      if(((int(cfg_entry[ITEM_SIZE][VALUE],16) + alignment_bytes) % int(ITEM_SIZE_ALIGN,16))):
        print_error_exit("\nERROR: config item alinment requested is not a multiple of " + ITEM_SIZE_ALIGN)

      #Try writing 0s to file and fix alignment. if the file is RO, report the error.
      try:
        #add 0 padding to the binary file in case of alignment issue
        print("Appending " + hex(int(alignment_bytes)).rstrip("L") + " 0s to: " + config_file + " to make the file " + align + " bytes aligned")
        with open(config_file, 'ab') as file:
            file.write(b'\x00'*(int(alignment_bytes)))
            file.close()
      except:
...
def create_meta_data_table(json_input_cfg_items, xbl_cfg_value, fp_cfg_meta_file, base_directory, align, store_original_size):
  # handle version check
  global cfg_item_json
  global major_version
  global minor_version
  major_version = json_input_cfg_items.pop("major_version", 1)
  minor_version = json_input_cfg_items.pop("minor_version", 0)
  if major_version < INFO_VERSION:
    cfg_item_json = cfg_item_json_v1
  else:
    cfg_item_json = cfg_item_json_v2
  ...
  for input_cfg_item_key in json_input_cfg_items:
    new_cfg_entry = create_new_cfg_item(json_input_cfg_items[input_cfg_item_key], base_directory, align, store_original_size)
    check_dup_cfg_name_and_error(meta_cfg_entries, new_cfg_entry)
    meta_cfg_entries.append(new_cfg_entry)
  calc_and_fill_cfg_entry_offset_and_meta_data_size(meta_cfg_entries, meta_header,align)
...
def generate_xbl_config_metadata(json_config_path, base_directory, output_directory, align, store_original_size):
  ...
  for xcfg_meta_item in input_json_xbl_cfgs:
    ...
    create_meta_data_table(input_json_xbl_cfgs[xcfg_meta_item], str(xcfg_meta_item), fp_cfg_meta_file, base_directory, align, store_original_size)
    fp_cfg_meta_file.close()

  unaligned_bytes = os.path.getsize(xcfg_meta_file) % int(align,16)
  ...
  if(unaligned_bytes):
    ...
    try:
      #add 0 padding to the binary file in case of alignment issue
      print("Appending " + hex(int(alignment_bytes)).rstrip("L") + " 0s to: " + xcfg_meta_file + " to make the file " + align + " bytes aligned")
      with open(xcfg_meta_file, 'ab') as file:
          file.write(b'\x00'*(int(alignment_bytes)))
          file.close()
    except:
      print_error_exit("/nERROR: Input file:\"" + xcfg_meta_file + "\" is not " + align + " aligned and RO. Please update input file size to be " + align + " bytes aligned")
...
def create_xcfg_json(cfg_meta_key,
  ...
  item_num = 1
  # Add Version fields first
  xcfg_dict = disassembled_xcfg_dict
  xcfg_dict[cfg_meta_key][MAJOR_VERSION] = version_dict[MAJOR_VERSION]
  xcfg_dict[cfg_meta_key][MINOR_VERSION] = version_dict[MINOR_VERSION]

  # statistics the number of segments
  for key in disassembled_bins_info.keys():
  ...
  for cfg_entry in meta_cfg_entries:
    ...
    #if it is single segment DTB elf, extract each config segment from cfg segment
    # otherwise copy it from corresponding segment
    if number_of_segments == 3 or number_of_segments == 1:
      if number_of_segments == 3:
        cfg_segment_name = 'segment_1'
      elif number_of_segments == 1:
        cfg_segment_name = 'segment_0'
      try:
        input_fp = open(os.path.join(disassembled_bins_path, disassembled_bins_info[cfg_segment_name]),mode='rb')
      except Exception as e:
        print("\nERROR: An error occurred while opening "+os.path.join(disassembled_bins_path, disassembled_bins_info[cfg_segment_name])+" file. Please check if it is existing!!")
        print(e)
        sys.exit(-1)
      try:
        output_fp = open( os.path.join(output_directory,cfg_item_filename), mode='wb+')
      except:
        print("\nERROR: An error occurred while creating " + os.path.join(output_directory,cfg_item_filename))
        sys.exit(-1)
      try:
        input_fp.seek(int(cfg_entry['offset_from_meta_start']['value'],0),0)
      except Exception as e:
        print("\n ERROR"+e)
        sys.exit(-1)
      try:
        output_fp.write(input_fp.read(int(cfg_entry['item_size']['value'],0)))
      except Exception as e:
        print("\nERROR: An error occurred while writing " + os.path.join(output_directory, cfg_item_filename))
        print(e)
        sys.exit(-1)
      input_fp.close()
      output_fp.close()
      ...
      print("copy ",os.path.join(disassembled_bins_path, disassembled_bins_info[info_key])," to ",os.path.join(output_directory,cfg_item_filename.rstrip('\0')))
      shutil.copyfile(os.path.join(disassembled_bins_path, disassembled_bins_info[info_key]),
                                   os.path.join(output_directory,cfg_item_filename.rstrip('\0')))
...
if __name__ == "__main__":
  ...
  if not options.disassembled_xcfg_info_json:
    ...
    generate_xbl_config_metadata(options.json_config_path, options.base_directory, options.output_directory, options.align, options.store_original_size)
  else:
