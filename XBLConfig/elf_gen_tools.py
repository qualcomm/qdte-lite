...
ELFINFO_RELOC_ETYPE       = b'\x01\x00' #************************************************
ELFINFO_DYNAM_ETYPE       = b'\x03\x00' #************************************************
ELFINFO_CORE_ETYPE       = b'\x04\x00' #************************************************
ELFINFO_NONE_ETYPE       = b'\x00\x00' #************************************************
...
def create_elf_header_gui( output_file_name,
   ...
   if (elf_struct.is_elf_64_bit == True):
      # ELf header
      ...
      elf_fp.write(''.rjust(9, chr(ELFINFO_RESERVED)).encode())
      ...
      elf_fp.write(''.rjust(4, chr(ELFINFO_RESERVED)).encode())
      ...
      elf_fp.write(''.rjust(6, chr(ELFINFO_RESERVED)).encode())
...
def image_header(env, gen_dict,
   ...
   if (gen_dict['IMAGE_KEY_MBN_TYPE'] == 'elf') and (elf_file_name == None):
...
def modify_elf_flags(env, elf_in_file_name,
   ...
   if len(segment_list) != (elf_header.e_phnum -sub):
...
def getSegmentFlag(seg_info):
   ...
   if seg_info == None or len(seg_info) == 0:
...
def generate_global_dict(env):
   ...
   if (os.path.exists(cust_file_name) == True) and \
      (os.path.exists(targ_file_name) == True):
