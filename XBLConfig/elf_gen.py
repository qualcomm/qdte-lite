...
def config_parser(json_file, gui_option):
  ...
  for i in range (elf_struct.e_phnum):
    ...
    physically_relocatable_type_var = 0x0
    access_type_var = 0x0
    ...
    if( int(data["program_header"]["segment"][i]["relocatable"],16) == 0x1):
      physically_relocatable_type_var = 0x08000000

    new_phdr.p_flags  = pflags_var | access_type_var | segment_type_var | physically_relocatable_type_var
...
def disassemble_elf_raw(elf_fp, input_elf_filename, elf_info_out_json, output_path):
  ...
  if elf_header.e_ident[ELFINFO_CLASS_INDEX] == int.from_bytes(ELFINFO_CLASS_32,byteorder='little'):
    elf_header_fp.write(elf_gen_tools.Elf32_Ehdr.getPackedData(elf_header))
    if isinstance(ELFINFO_CLASS_32,bytes):
      elf_class = ELFINFO_CLASS_32
    else:
      elf_class = str(ELFINFO_CLASS_32)
  else:
    elf_header_fp.write(elf_gen_tools.Elf64_Ehdr.getPackedData(elf_header))
    if isinstance(ELFINFO_CLASS_64,bytes):
      elf_class = ELFINFO_CLASS_64
    else:
  ...
  if sys.version_info >= (3, 0, 0):
    if isinstance(elf_class,bytes):
      elf_info_dict[E_CLASS] = hex(int.from_bytes(elf_class,byteorder='little', signed=False))
    else:
...
if __name__ == "__main__":
  ...
  parser.add_option("-a", "--align_bytes",
  ...
  if options.align_bytes:
    PAGE_SIZE = int(options.align_bytes,16)
