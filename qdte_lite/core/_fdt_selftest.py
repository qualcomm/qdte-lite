...
def _stream(backend, fdt):
    ...
    return [
        (path, _classify(backend, item), _value(backend, item))
...
def check_parity(paths):
    ...
    for path in paths:
        ...
        assert len(ours) == len(theirs), "%s: item count %d != %d" % (
            path,
            len(ours),
            len(theirs),
        )
...
def check_roundtrip(paths):
    for path in paths:
        ...
        target = next(
            (
                p
                for p, item in fdt.resolve_path("/").walk()
                if isinstance(item, fdt_backend.FdtPropertyWords)
            ),
            None,
        )
        ...
        for p, (start, end) in mappings.items():
            assert 0 <= start < end <= len(blob_a), "%s: bad mapping for %s" % (path, p)
