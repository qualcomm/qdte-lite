...
class assemble:
    def fetch_gendtbelf_script(self):
        ...
        if os.path.exists(os.path.join(gf["xbltoolsDir"], "GenConfigImage.py")):
            ...
            raise Exception(
                "ERROR: Gen DTB tools does not exist",
                "Please choose correct GEN DTB tools path!!\n",
            )
        return tool_path
    ...
    def load_disassembled_elf_info(self):
        ...
        try:
            with open(
                os.path.join(self.outdir, "disassembled_elf_info.json"),
                mode="r",
                encoding="utf-8",
            ) as fp:
    ...
    def check_alignment_setting(self):
        try:
            with open(
                os.path.join(self.outdir, "auto_genphdr.bin"),
                mode="r",
                encoding="utf-8",
            ) as fp:
                self.disassembled_elf_info = json.load(fp)
    ...
    def dissamble_config_elf(self):
        ...
        if self.is_compressed_elf(gf["inputFile"]):
            ...
            decompressed_output_file = os.path.join(
                self.outdir, os.path.basename(gf["inputFile"])
            )
        ...
        ## dissamble elf file
        return_code = version_2_assemble.disassemble_version_2_elf(
            gf["inputFile"],
            self.outdir,
            self.outdir,
        ...
        if gf["inputFile"] is None or not os.path.exists(gf["inputFile"]):
            raise Exception(
                "Error: input path {}.{}".format(gf["inputFile"], " is invalid path!!")
            )
        try:
        ...
        while True:
            ...
            for line in outs.split("\n"):
                ...
                if "Error:" in line or "ERROR" in line or "Errno" in line:
                    ...
                    if (
                        "GenXBLConfig.py" in line
                        and "No such file or directory" in line
                    ):
                        hints.append("Are you sure the Config tool is valid?")

                else:
                    if (
                        "XBLconfig_metadata_generator.py: Couldn't find XBL config"
                        in line
                    ):
        ...
        dtbs_bin_list = [
            file for file in os.listdir(self.outdir) if file.endswith("dtbs.bin")
        ]
        for dtbs_bin in dtbs_bin_list:
            ...
            version_2_assemble.disassembled_dtbs_to_separate_files(
                dtbs_file, meta_entry_json, dtbs_folder_name
            )
            os.remove(dtbs_file)
    ...
    def reassemble_config_elf(self):
        ...
        if "create_xbl_config.json" not in os.listdir(self.outdir):
            ...
            if not os.path.exists(
                os.path.join(self.outdir, "auto_gen", "elf_files", "create_cli")
            ):
                os.mkdir(
                    os.path.join(self.outdir, "auto_gen", "elf_files", "create_cli")
                )
            ...
            desired_name = version_2_assemble.reassemble_version_2_elf(
                self.outdir,
                os.path.join(self.outdir, "auto_gen", "elf_files", "create_cli"),
                "",
                gf["outputFile"][:-4]
                if gf["outputFile"].endswith(".elf")
                else gf["outputFile"],
                True,
            )
        ...
        dtb_dtbs_folder = [
            folder for folder in os.listdir(self.outdir) if folder.endswith(".dtbs")
        ]
        for dtbs_folder in dtb_dtbs_folder:
            ...
            if os.path.exists(dtbs_file):
                os.remove(dtbs_file)
                dtbs_file = ""
            dtbs_file = version_2_assemble.reassemble_output_dtb_dtbs(
                os.path.join(self.outdir, dtbs_folder), self.outdir
            )
            if dtbs_file != "":
        ...
        cmd = [
            ...
            self.outdir,
            "-i",
            os.path.join(self.outdir, "create_xbl_config.json"),
            "-b",
            self.outdir,
            "-f",
            ELF_FORMATS[int(self.disassembled_elf_info["E_CLASS"], 16)],
            "-o",
            gf["outputFile"][:-4]
            if gf["outputFile"].endswith(".elf")
            else gf["outputFile"],
            "-x",
            self.disassembled_elf_info["alignment"],
        ]
